...
<<<<<<< HEAD
      const hashedPassword = await bcryptjs.hash(user.password, 10).catch((err) => {
=======
...
>>>>>>> 8e2c81faa9697c85a16363085bb95824ff6fb2a0
      ...
      this.pool.getConnection((err, connection) => {
        ...
        const query = `insert into users( name, card_id, password, birth_date, grade, telephone_number, role, nis, is_active, inactive_reason, gender, class_type, origin, residence_in_semarang) values ('${user.name
          .split("'")
          .join("''")}',${user.card_id ? `'${user.card_id}'` : null}, '${hashedPassword}', '${user.birth_date}', ${user.grade ? `'${user.grade}'` : null}, '${user.telephone_number}', '${user.role}', ${user.nis ? `'${user.nis}'` : null}, ${
          user.is_active ? `'${user.is_active}'` : null
        }, ${user.inactive_reason ? `'${user.inactive_reason}'` : null}, ${user.gender}, ${user.class_type ? `'${user.class_type}'` : null}, ${user.origin ? `'${user.origin}'` : null}, ${
          user.residence_in_semarang ? `'${user.residence_in_semarang}'` : null
        })`;
  ...
  updateUser = async (user, withUpdatePassword) =>
    new Promise(async (resolve, reject) => {
<<<<<<< HEAD
      const hashedPassword = await bcryptjs.hash(user.password, 10).catch((err) => {
        reject({ msg: "Bcryptjs error." });
=======
      const hashedPassword = await bcrypt.hash(user.password, 10).catch((err) => {
