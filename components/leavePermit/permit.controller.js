...
class PermitController {
  ...
  createPermit = async (req, res) => {
    const permit = new Permit(req.userData.id, req.body.class_id, req.body.description, `/${this.env.fileUploadPath}/${req.file.filename}`, 0);
  ...
  updatePermit = async (req, res) => {
    ...
    if (req.userData.role === 1 && !managedClass) {
      const permit = new Permit(req.userData.id, req.body.class_id, req.body.description, req.body.img_url, req.body.isApproved);
      try {
    ...
    const permit = new Permit(req.body.user_id, req.body.class_id, req.body.description, req.body.img_url, req.body.isApproved);

    const attendance = new Attendance(req.body.user_id, req.body.class_id, moment().format("YYYY-MM-DD HH:mm:ss"), "izin", req.userData.id);
