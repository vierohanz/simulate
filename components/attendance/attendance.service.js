...
class AttendanceService {
  ...
  getAttendances = (returnAll, sortByAttend_at, start_date, end_date) =>
    new Promise((resolve, reject) => {
      this.pool.getConnection((err, connection) => {
        if (err) {
          reject({ msg: "Could not connect to the database." });
        ...
        if (returnAll) {
          query = `select a.user_id, a.class_id, u2.nis, u2.name, u2.grade, u2.gender, c.name as class_name, ct.name as class_type, c.start_date, c.end_date, a.attend_at, a.status, IFNULL(ELT(FIELD(u.role, 1, 2, 3), 'santri', 'guru', 'admin'), 'sistem') as lastEditBy from attendances a left join classes c on a.class_id = c.id  left join users u on a.lastEditBy = u.id join users u2 on a.user_id = u2.id left join class_types ct on u2.class_type = ct.id ${
            sortByAttend_at ? `order by a.attend_at desc` : ""
          }`;
        } else if (start_date && end_date) {
          query = `select a.user_id, a.class_id, u2.nis, u2.name, u2.grade, u2.gender, c.name as class_name, ct.name as class_type, c.start_date, c.end_date, a.attend_at, a.status, IFNULL(ELT(FIELD(u.role, 1, 2, 3), 'santri', 'guru', 'admin'), 'sistem') as lastEditBy from attendances a left join classes c on a.class_id = c.id  left join users u on a.lastEditBy = u.id join users u2 on a.user_id = u2.id left join class_types ct on u2.class_type = ct.id where c.start_date between '${start_date}' and '${end_date}' ${
            sortByAttend_at ? `order by a.attend_at desc` : ""
          }`;
        } else if (start_date) {
          query = `select a.user_id, a.class_id, u2.nis, u2.name, u2.grade, u2.gender, c.name as class_name, ct.name as class_type, c.start_date, c.end_date, a.attend_at, a.status, IFNULL(ELT(FIELD(u.role, 1, 2, 3), 'santri', 'guru', 'admin'), 'sistem') as lastEditBy from attendances a left join classes c on a.class_id = c.id  left join users u on a.lastEditBy = u.id join users u2 on a.user_id = u2.id left join class_types ct on u2.class_type = ct.id where c.start_date > '${start_date}' ${
            sortByAttend_at ? `order by a.attend_at desc` : ""
          }`;
        } else {
  ...
  getAttendanceRecap = (start_date, end_date, class_type) =>
    new Promise((resolve, reject) => {
      this.pool.getConnection((err, connection) => {
        if (err) {
          reject({ msg: "Could not connect to the database." });
  ...
  getAttendancesByUser = (userId, limit) =>
    new Promise((resolve, reject) => {
      this.pool.getConnection((err, connection) => {
        if (err) {
          reject({ msg: "Could not connect to the database." });
  ...
  getAttendanceByUserAndClass = (userId, classId) =>
    new Promise((resolve, reject) => {
      this.pool.getConnection((err, connection) => {
        if (err) {
          reject({ msg: "Could not connect to the database." });
  ...
  deleteAttendance = (userId, classId) =>
    new Promise((resolve, reject) => {
      this.pool.getConnection((err, connection) => {
        if (err) {
          reject({ msg: "Could not connect to the database." });
